...
export type RivalSort = (typeof RIVAL_SORTS)[number]["key"];

export const DEFAULT_RIVAL_SORT: RivalSort = "games";

export function isRivalSort(v: string | null | undefined): v is RivalSort {
  return RIVAL_SORTS.some((s) => s.key === v);
}

/** 정렬에 필요한 것만 본다 — `RivalRow` 전체를 요구하면 테스트가 못 쓴다. */
export interface SortableRival {
  vs_matches: number;
...
export function versusRecord(r: SortableRival): HeadToHead {
  return {
...
const played = (r: SortableRival) => r.vs_matches + r.ally_matches;
 ...
 */
export function sortRivals<T extends SortableRival>(rows: T[], sort: RivalSort): T[] {
  const out = [...rows];
  if (sort === "recent") {
