...
import { DEFAULT_RIVAL_SORT, isRivalSort, sortRivals, versusRecord } from "./rivals.ts";
...
test("판수순 — 맞붙은 판과 같은 팀 판을 합쳐 센다", () => {
  ...
  assert.deepEqual(names(sortRivals(rows, "games")), ["많이", "중간", "적게"]);
});
...
test("★ 맞붙은 적 없는 상대는 승률순에서 맨 뒤로 간다", () => {
  ...
  assert.deepEqual(names(sortRivals(rows, "winrate")), ["잘이김", "반타작", "같은팀만"]);
});
...
test("무승부는 승률 분모에서 빠진다", () => {
  ...
  assert.deepEqual(names(sortRivals(rows, "winrate")), ["무승부낀쪽", "반타작"]);
});

test("원본 배열을 건드리지 않는다", () => {
  const rows = [r("a", { vs: 1 }), r("b", { vs: 9 })];
  sortRivals(rows, "games");
  assert.deepEqual(names(rows), ["a", "b"]);
});

test("주소창에 아무거나 넣어도 기본값으로 간다", () => {
  assert.equal(isRivalSort("winrate"), true);
  assert.equal(isRivalSort("아무말"), false);
  assert.equal(isRivalSort(undefined), false);
  assert.equal(DEFAULT_RIVAL_SORT, "games");
});
