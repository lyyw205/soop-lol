 ...
 * ⚠ **이건 분류기가 아니라 거름망이다.**
 *
 * `gameRatio` 가 답하는 건 "게임 같은 화면이 얼마나 있었나" 지 "롤이었나" 가 아니다.
 * 192×108 썸네일로는 게임 종류를 못 가른다 — 여러 각도로 실측해 확인했다
 * (2026-08-12, 이상호 8월 표본 7개):
 *
 ...
 *
 * FC온라인은 잔디가 어둡고 유니폼이 알록달록해서 `밝기<75 && 채도>28` 을 그대로
 * 통과한다. 실측 오탐률 **67%**(6건 중 4건). 임계값을 조이면 진짜 롤을 버린다.
 *
 * → **판정은 원본 해상도 프레임 한 장**이 한다. 거름망이 후보를 줄이고(60채널 12일 → 6건),
 *   확인 프레임 한 장(6MB)이 게임 종류를 확정한다. `confirmShot` 참조.
 */
...
/**
 * **확인 프레임 지점** — 경기 구간마다 한가운데를 하나씩.
 *
 * 구간 한가운데가 게임이 가장 확실히 진행 중인 자리다 — 로딩·결과창·알트탭이 아니라
 * 실제 플레이 화면이라, 원본 해상도로 받으면 무슨 게임인지 한눈에 갈린다.
 *
 * ⚠ **VOD 하나에 한 장으로 줄이지 마라.** 5시간 방송이면 게임을 갈아탄다.
 * 실제로 2026-08-08 "이상호 피파" VOD 안에 롤 내전이 두 개 들어 있었다.
 * 가장 긴 구간 한 장만 봤으면 FC온라인으로 판정하고 통째로 버렸을 것이다.
 * 긴 구간부터 `max` 장까지 — 구간당 6MB 라 후보 전부에 붙여도 부담이 없다.
 */
export function confirmShots(games, max = 6) {
  return games
    .slice()
