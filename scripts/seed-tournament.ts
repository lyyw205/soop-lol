...
interface SeedGame {
   ...
   *
   * ★ **무엇을 한 시리즈로 묶을지는 결과 화면만 봐서는 못 정한다.**
   *   CK 는 승부가 난 뒤 장난식 추가 세트를 더 한다(미드 바꿔서 단판·범인찾기…).
   *   그걸 세트로 이어 붙이면 2:0 으로 끝난 시리즈가 2:2 가 된다.
   *   랜드는 매 판 팀을 섞으므로 애초에 시리즈가 아니다.
   *   스크림은 시간 경계에서 하던 판을 전원 탈주하는데 그 판은 전적이 아니다.
   *   → **docs/CK-COLLECTION.md §3.5 를 읽고 정한다.**
   */
  series?: string;
  set_no?: number;
  played_at: string;
