...
**M1 코드 완성.** 스키마·`RiotClient`·지표 계산·관리자 화면에 더해
**수집 워커(Engine A~D, `apps/worker`)까지 만들고 검증했다.**
남은 건 **실제 데이터 투입** — 키가 없어서 아직 한 줄도 안 긁었다.

**다음은 M2 화면** (`/s/[slug]`, `/vs/[a]/[b]`) — docs/PLAN.md §7, §10.

막혀 있는 것 두 개 (docs/SETUP.md):
- Riot API 키가 아직 없다 (Personal 키 신청은 승인까지 수일~수주 → 빨리 넣을수록 좋다)
- Supabase 무료 플랜 활성 프로젝트 2개 한도에 걸려 프로젝트를 못 만들었다
